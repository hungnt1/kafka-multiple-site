...
Secure SSL
```
ansible-playbook -i inventory/<environment>/cluster.ini createUserSSL.yml
...
**Play only on site HN**


- Copy des site ( as HCM ) clusteraction tls (default is broker public tls)
It will use for 
```
kafka-mirror-maker-consumer.properties

root@HCM: 
scp /usr/local/public-broker.truststore.jks root@192.168.30.190:/usr/local/public-broker-hcm.truststore.jks
scp /usr/local/public-broker.truststore.jks root@192.168.30.191:/usr/local/public-broker-hcm.truststore.jks
scp /usr/local/public-broker.truststore.jks root@192.168.30.192:/usr/local/public-broker-hcm.truststore.jks

scp /usr/local/public-broker.keystore.jks root@192.168.30.190:/usr/local/public-broker-hcm.keystore.jks
scp /usr/local/public-broker.keystore.jks root@192.168.30.191:/usr/local/public-broker-hcm.keystore.jks
scp /usr/local/public-broker.keystore.jks root@192.168.30.192:/usr/local/public-broker-hcm.keystore.jks
```
...
ansible-playbook -i inventory/development/cluster.ini clusterKafkaMirrorMaker.yml
```
...
## Client create for prod
```

ansible-playbook -i inventory/<environment>/cluster.ini creatUserSSL.yml
